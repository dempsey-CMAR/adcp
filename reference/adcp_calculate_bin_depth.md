...
- dat:
  ...
  Data frame of ADCP data in long format, as returned by
  `adcp_pivot_longer()`.

- metadata:

  Data frame with metadata information for the deployment in `dat`
  (e.g., a row from CURRENT & WAVE TRACKING). Must include column
  `sensor_height_above_sea_floor_m` (formerly Inst_Alt). Option to use
  default value `metadata = NULL` and provide the required value in the
  `inst_alt` argument.
