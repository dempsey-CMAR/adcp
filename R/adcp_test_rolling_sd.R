...
#'
#' @param current_rolling_sd_table Data frame with at least two columns:
...
#'
#' @param period_hours Length of a full cycle in hours. Default assumes a tidal
...
#' @family tests
#'
#' @inheritParams adcp_pivot_vars_longer
#'
...
adcp_test_rolling_sd <- function(
 ...
 # county <- assert_county(dat, county, "qc_test_rolling_sd()")
  ...
  if (is.null(current_rolling_sd_table)) {

    current_rolling_sd_table <- current_thresholds %>%
  ...
  dat <- dat %>%
    ...
    left_join(current_rolling_sd_table, by = join_by(variable)) %>%
