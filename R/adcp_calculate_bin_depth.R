...
#'
#' @param dat Data frame of ADCP data in long format, as returned by
#'   \code{adcp_pivot_longer()}.
#'
#' @param metadata Data frame with metadata information for the deployment in
#'   \code{dat} (e.g., a row from CURRENT & WAVE TRACKING). Must include column
#'   \code{sensor_height_above_sea_floor_m} (formerly Inst_Alt). Option to use
#'   default value \code{metadata = NULL} and provide the required value in the
#'   \code{inst_alt} argument.
#'
...
adcp_calculate_bin_depth <- function(dat,
                                     metadata = NULL,
                                     inst_alt = NULL) {
  if (!is.null(metadata)) inst_alt <- metadata$sensor_height_above_sea_floor_m
