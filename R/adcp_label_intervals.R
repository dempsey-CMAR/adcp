...
#'
#' Assigns intervals to \code{column} using the \code{cut()} function. Intervals
#' start and 0 and end at the maximum value that gives a \code{n_ints} equal
#' intervals. This means some small and/or large intervals may have zero
#' observations.
#'
...
#'
#'  @param min_value Minimum value for the first interval. The default is zero.
#'
#' @param max_value Maximum value for the last interval. The default is the
#'   smallest value that will give \code{n_ints} equal intervals (starting from
#'   0). This should be set to 360 degrees for direction data.
#'
#' @param label_sep Separator for the interval labels ("lower to upper").
#'   Default is a new line to save room on plot axis.
#'
#' @return Returns \code{dat} with an additional column \code{int_label}, the
#'   interval labels as an ordered factor.
#'
...
adcp_label_intervals <- function(
  ...
  dat$ints <- cut(
    dat$col_to_cut,
    breaks =  seq(min_value, max_value, cut_width),
  ...
  dat  %>%
    mutate("ints_{{column}}" := factor(ints_label, levels = ints_levels)) %>%
