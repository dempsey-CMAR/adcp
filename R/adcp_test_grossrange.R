...
#'
#' @param current_grossrange_table Data frame with at least 5 columns:
...
#'   was collected. Used to filter the default \code{current_grossrange_table}.
#'
...
adcp_test_grossrange <- function(
  ...
  if (is.null(current_grossrange_table)) {

    current_grossrange_table <- current_thresholds %>%
  ...
  dat <- dat %>%
    ...
    left_join(current_grossrange_table, by = join_by(variable)) %>%
    mutate(
